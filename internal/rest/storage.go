...
func (h *Handler) PostFile(c *fiber.Ctx) error {
	...
	filePath := fmt.Sprintf("./tmp/upload/minio_%s", name)
	...
	defer func() {
		...
		if err != nil {
			slog.Info("failed to remove temporary file", "path", filePath)
		}
	}()

	if err = h.Services.PostFile(ctx, filePath); err != nil {
...
func (h *Handler) GetFileContent(c *fiber.Ctx) error {
	...
	return c.JSON(FileContentResponse{Content: content.Content})
}
